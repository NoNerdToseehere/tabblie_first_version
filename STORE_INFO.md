...
- `notifications/order-refund.liquid` is a Tabblie-styled refund e-mail built from the confirmation blocks (no Klaviyo original): uses `amount`, `refund_line_items` (`rl.line_item`, `rl.quantity`), `order_name`, `order_status_url`. Paste into Customer notifications → Order refund, subject "Je terugbetaling is geregeld".
...
- **Handover 2026-09-06:** all repo work up to this commit is pushed and live; the founder now edits directly in the Shopify theme editor on the connected theme. Those edits come back as "Update from Shopify" commits on `main`. **Before any further repo work: `git pull` first and read those commits**, otherwise the next push overwrites editor changes.
...
- **Storefront is publicly live** (password removed, verified 2026-08-24) — content can be checked directly at tabblie.com. Remember: every template file in the published theme is publicly reachable via `?view=<suffix>`, so old template versions must be pruned, not parked.
